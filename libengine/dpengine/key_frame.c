...
bool DP_key_frame_layer_hidden(DP_KeyFrameLayer *kfl)
{
    DP_ASSERT(kfl);
    return (kfl->flags & LAYER_VISIBILITY_MASK) == DP_KEY_FRAME_LAYER_HIDDEN;
}

bool DP_key_frame_layer_revealed(DP_KeyFrameLayer *kfl)
{
    DP_ASSERT(kfl);
...
{
    DP_TransientKeyFrame *tkf = allocate_key_frame(true, 0);
    tkf->title = NULL;
