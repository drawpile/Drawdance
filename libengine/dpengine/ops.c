                                    ...
                                    const char *title, size_t title_length)
{
    ...
    if (insert_id != 0 && index == track_count) {
                                   ...
                                   int (*get_track_id)(void *, int), void *user)
{
    ...
    for (int i = 0; fill < track_id_count && i < track_id_count; ++i) {
        DP_Track *t = DP_timeline_at_noinc(tl, i);
        int track_id = DP_track_id(t);
        if (DP_transient_timeline_index_by_id(ttl, track_id) == -1) {
            DP_transient_timeline_set_inc(ttl, t, fill);
                                     ...
                                     int frame_index, int layer_id)
{
    ...
    if (frame_index >= frame_count) {
        DP_error_set("Key frame set: frame index %d beyond frame count %d",
    ...
    if (t_index == -1) {
        DP_error_set("Key frame set: track id %d not found", track_id);
        return NULL;
    }
    ...
    if (layer_id != 0) {
        DP_LayerRoutes *lr = DP_canvas_state_layer_routes_noinc(cs);
        if (!DP_layer_routes_search(lr, layer_id)) {
            DP_error_set("Key frame set: layer id %d not found", layer_id);
            return NULL;
        }
    }
    ...
    if (replace) {
        ...
        DP_transient_track_insert_transient_noinc(
            tt, frame_index, DP_transient_key_frame_new_init(layer_id),
                                        ...
                                        int frame_index)
{
    ...
    DP_transient_track_delete_at(tt, kf_index);
