...
static void add_layer(DP_Output *output, DP_CanvasHistory *ch,
                      DP_DrawContext *dc, uint16_t id, const char *name,
                      bool group, uint16_t parent_id)
{
    uint8_t flags = (group ? DP_MSG_LAYER_TREE_CREATE_FLAGS_GROUP : 0)
                  | (parent_id == 0 ? 0 : DP_MSG_LAYER_TREE_CREATE_FLAGS_INTO);
...
static void add_key_frame_set(DP_Output *output, DP_CanvasHistory *ch,
                              ...
                              uint16_t frame_index, uint16_t layer_id)
{
    add_message(output, ch, dc,
                DP_msg_key_frame_set_new(1, track_id, frame_index, layer_id));
}
...
static void set_layers(int count, uint16_t *out, void *user)
{
    int *layers = user;
    for (int i = 0; i < count; ++i) {
        out[i] = DP_int_to_uint16(layers[i]);
    }
}
                                           ...
                                           uint16_t frame_index, int *layers)
{
    ...
    add_message(output, ch, dc,
                DP_msg_key_frame_layer_attributes_new(
                    1, track_id, frame_index, set_layers, count, layers));
}
                            ...
                            DP_DrawContext *dc)
{
    ...
    add_layer(output, ch, dc, 257, "L1", false, 0);
    add_layer(output, ch, dc, 258, "G2", true, 0);
    add_layer(output, ch, dc, 259, "G2/L1", false, 258);
    add_layer(output, ch, dc, 260, "G2/L2", false, 258);
    add_layer(output, ch, dc, 261, "G2/G3", true, 258);
    add_layer(output, ch, dc, 262, "G2/G3/L1", false, 261);
    add_layer(output, ch, dc, 263, "G2/G3/L2", false, 261);
    add_layer(output, ch, dc, 264, "L3", false, 0);
    add_layer(output, ch, dc, 265, "L4", false, 0);
    add_undo_point(output, ch, dc);
    ...
    add_undo_point(output, ch, dc);
    add_key_frame_set(output, ch, dc, 300, 20, 258);
    ...
    add_undo_point(output, ch, dc);
    add_key_frame_delete(output, ch, dc, 300, 20);
    ...
    add_undo_point(output, ch, dc);
    add_key_frame_delete(output, ch, dc, 300, 29);
    dump_timeline(output, ch, "delete track 1 key 29", false);

    add_undo_point(output, ch, dc);
    add_key_frame_delete(output, ch, dc, 300, 0);
    dump_timeline(output, ch, "delete track 1 key 0", false);

    add_undo_point(output, ch, dc);
    add_key_frame_delete(output, ch, dc, 300, 10);
