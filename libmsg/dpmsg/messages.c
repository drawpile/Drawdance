                                                  ...
                                                  unsigned char *data)
{
    ...
    written += DP_write_bigendian_uint16(mkfs->layer_id, data + written);
                                                 ...
                                                 DP_TextWriter *writer)
{
    ...
    return DP_text_writer_write_uint(writer, "frame_index", mkfs->frame_index,
                                     true)
        && DP_text_writer_write_uint(writer, "layer_id", mkfs->layer_id, true)
                                             ...
                                             size_t length)
{
    ...
    uint16_t layer_id = read_uint16(buffer + read, &read);
                                       ...
                                       DP_TextReader *reader)
{
    ...
    uint16_t frame_index = (uint16_t)DP_text_reader_get_ulong_hex(
        reader, "frame_index", UINT16_MAX);
    uint16_t layer_id =
        (uint16_t)DP_text_reader_get_ulong_hex(reader, "layer_id", UINT16_MAX);
    return DP_msg_key_frame_set_new(context_id, track_id, frame_index,
                                    layer_id);
}
...
uint16_t DP_msg_key_frame_set_layer_id(const DP_MsgKeyFrameSet *mkfs)
{
    DP_ASSERT(mkfs);
    return mkfs->layer_id;
}
